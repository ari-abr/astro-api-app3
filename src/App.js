...
class App extends Component {
  ...
  // Used to be functional for external API calls, updated to use JSON data within app files for easier maintenance
  /*
  async getResults(option) {
  this.setState({ results: null }, () =>
    console.log('null results :( ' + this.state.results)
  );
  await fetch(planetsData)
    .then((response) => {
  ...
  async getResults(option) {
    ...
    await this.findResultsArray(option);
    var myResult = this.findResultsArray(option);
    this.setState({ results: myResult }, () =>
      console.log(this.state.results)
    );
  };
